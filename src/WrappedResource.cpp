	...
	resourcePtr(native), resource11(shared)
{
	...
	if (desc.BindFlags & D3D11_BIND_SHADER_RESOURCE) {
		...
		DX::ThrowIfFailed(a_d3d11Device->CreateShaderResourceView(shared, &srvDesc, &srv));
	}
	...
	if (desc.BindFlags & D3D11_BIND_UNORDERED_ACCESS) {
		if (desc.ArraySize > 1) {
			...
			DX::ThrowIfFailed(a_d3d11Device->CreateUnorderedAccessView(shared, &uavDesc, &uav));
		}
		else {
			D3D11_UNORDERED_ACCESS_VIEW_DESC uavDesc = {};
			uavDesc.Format = desc.Format;
	...
	if (desc.BindFlags & D3D11_BIND_RENDER_TARGET) {
		...
		rtvDesc.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2D;
		rtvDesc.Texture2D.MipSlice = 0;
		DX::ThrowIfFailed(a_d3d11Device->CreateRenderTargetView(shared, &rtvDesc, &rtv));
	}
}
